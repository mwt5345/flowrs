...
impl NSF {
    #[new]
    #[pyo3(signature = (d_input, num_flows, hidden_sizes, num_bins=8, tail_bound=3.0, seed=42))]
    fn new(
        d_input: usize,
        num_flows: usize,
        hidden_sizes: Vec<usize>,
        ...
        let device = Default::default();
        let config = flowrs::NsfConfig::new(d_input, num_flows, hidden_sizes.clone())
...
impl RealNVP {
    #[new]
    #[pyo3(signature = (d_input, num_flows, hidden_sizes, seed=42))]
    fn new(d_input: usize, num_flows: usize, hidden_sizes: Vec<usize>, seed: u64) -> Self {
        let device = Default::default();
        let config = flowrs::RealNvpConfig::new(d_input, num_flows, hidden_sizes.clone());
        let model = config.init::<B>(&device);
        RealNVP {
            model: Mutex::new(model),
            d_input,
    ...
    /// Train the RealNVP. Returns TrainHistory.
    #[pyo3(signature = (
        x, x_val=None,
        num_steps=20000, batch_size=512, lr=5e-4,
        noise_std=0.0, weight_decay=0.0,
    ...
    fn fit(
        ...
        let config = flowrs::RealNvpConfig::new(d, self.num_flows, self.hidden_sizes.clone());
        let mut ad_model = config.init::<AB>(&self.device);
